...
namespace Kenshin
{
	Shader::Shader(const std::string& vertex, const std::string& fragment, const std::string& geometry):m_RendererID(0)
	{
		int success;
	...
	void Shader::SetInt(const std::string& key, int val) const
	{
		glUniform1i(GetUniformLocation(key), val);
	}

	void Shader::SetFloat(const std::string& key, float val) const
	{
		glUniform1f(GetUniformLocation(key), val);
	}

	void Shader::SetVec2(const std::string& key, const glm::vec2& v2) const
	{
		glUniform2fv(GetUniformLocation(key), 1,  glm::value_ptr(v2));
	}

	void Shader::SetVec3(const std::string& key, const glm::vec3& v3) const
	{
		glUniform3fv(GetUniformLocation(key), 1, glm::value_ptr(v3));
	}

	void Shader::SetVec4(const std::string& key, const glm::vec4& v4) const
	{
		glUniform4fv(GetUniformLocation(key), 1, glm::value_ptr(v4));
	}

	void Shader::SetMat2(const std::string& key, const glm::mat2& m2) const
	{
		glUniformMatrix2fv(GetUniformLocation(key), 1, false, glm::value_ptr(m2));
	}

	void Shader::SetMat3(const std::string& key, const glm::mat3& m3) const
	{
		glUniformMatrix3fv(GetUniformLocation(key), 1, false, glm::value_ptr(m3));
	}

	void Shader::SetMat4(const std::string& key, const glm::mat4& m4) const
	{
		glUniformMatrix4fv(GetUniformLocation(key), 1, false, glm::value_ptr(m4));
	}

	int Shader::GetUniformLocation(const std::string& key) const
	{
